...
class CartController extends Controller
{
    ...
    private function currentCart()
    {
        ...
        $cartItems->each(function ($item) {
            ...
            // product main image
            if ($item->product && !isset($options['image'])) {
                $options['image'] = $item->product->thumb_image;
            }

            // customization  image + extra_price load
            if (!empty($options['customization_id'])) {
                $customization = CustomerCustomization::find($options['customization_id']);

                if ($customization) {
                    $options['front_image'] = $customization->front_image;
                    $options['back_image'] = $customization->back_image;
                    $options['extra_price'] = $customization->price ?? 0;
                }
            }

            $options['variant_total'] = $options['variant_total'] ?? 0;
    ...
    private function applyPromotions()
    {
        ...
        foreach ($promotions as $promo) {
            ...
            // if free item found add free product
            if (!$freeItem) {
    ...
    private function cartResponse()
    {
        ...
        return response()->json([
            'cart_items' => $cartItems->values()->map(function ($item) {
                return [
                    'id' => $item->id,
    ...
    private function cleanupPromotions()
    {
        ...
        foreach ($promotions as $promo) {
            ...
            // only remove free product if condition fails
