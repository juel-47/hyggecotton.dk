...
// const CustomizeProduct = ({ product, customization_id }) => {
const CustomizeProduct = ({ product}) => {
    // const { customization_id } = usePage().props;
    // console.log("Customization ID:", customization_id);
    ...
    const handleUpdateProduct = async () => {
        ...
        try {
            ...
            if (!product?.customization?.front_image && isFrontCustomized)
                throw new Error("Front base image is missing.");
            if (!product?.customization?.back_image && isBackCustomized)
            ...
            const customizePayload = {
                ...
                front_price: isFrontCustomized
                    ? product?.customization?.front_price || 4
                    : 0,
                back_price: isBackCustomized
                    ? product?.customization?.back_price
                    : 0,
                both_price:
                    isFrontCustomized && isBackCustomized
                        ? product?.customization?.both_price || 8
                        : 0,
            ...
            const price =
                isFrontCustomized && isBackCustomized
                    ? product?.customization?.both_price || 0
                    : 0;

            router.post(route("product-customize.store"), customizePayload, {
                preserveState: true,
                preserveScroll: true,
                onSuccess: (page) => {
                    // এখানে নতুন customization_id পাবে props থেকে
                    const newCustomizationId = page.props.customization_id;

                    if (!newCustomizationId) {
                        toast.error("কাস্টমাইজেশন সেভ হয়নি!");
                        return;
                    }

                    // এখন কার্টে অ্যাড করো
                    const cartPrice =
                        isFrontCustomized && isBackCustomized
                            ? product?.customization?.both_price || 8
                            : isFrontCustomized
                            ? product?.customization?.front_price || 4
                            : product?.customization?.back_price || 4;
                    ...
                    router.post(
                        route("cart.add"),
                        {
                            product_id: product?.id,
                            qty: 1,
                            customization_id: newCustomizationId,
                            price: cartPrice,
                        },
                        {
                            onSuccess: () => {
                                toast.success("সফলভাবে কার্টে যোগ হয়েছে!");
                                router.visit(route("cart.index"), {
                                    preserveState: false, // নতুন পেজ, পুরানো স্টেট দরকার নেই
                ...
                onError: (errors) => {
                    console.log(errors);
                    toast.error("কাস্টমাইজেশন সেভ করতে সমস্যা হয়েছে!");
                },
                onFinish: () => {
                    setIsUpdating(false);
                },
    ...
    const openPreview = async () => {
        try {
            ...
            // Safely get the customization object
            const customization = product?.customization;

            // Check if currentSide image exists
            const imageKey = `${currentSide}_image`;
            if (!customization || !customization[imageKey]) {
                toast.error(`No ${currentSide} image available.`);
                return;
            }

            // Proceed with preview logic
            // Example: open modal or set preview state
            setPreviewImage(customization[imageKey]);
