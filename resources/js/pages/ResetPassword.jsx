...
const ResetPassword = () => {
    const { props } = usePage();
    const { token, email: emailFromProps } = props; // Inertia passes token & email via props
    ...
    // ইমেইল প্রপস থেকে সেট করা
    useEffect(() => {
        if (emailFromProps) {
            setValue("email", emailFromProps);
        }
    }, [emailFromProps, setValue]);

    // লিঙ্ক ভ্যালিড কিনা চেক
    useEffect(() => {
        if (!token || !emailFromProps) {
            setIsValidLink(false);
        }
    }, [token, emailFromProps]);
        ...
        router.post(
            ...
            },
            {
                ...
                onSuccess: (page) => {
                    ...
                    reset();
                    setTimeout(() => router.visit(route("customer.login")), 2000);
                },
                onError: (errors) => {
                    ...
                    if (errors.message) {
                        ...
                        // Individual field errors (if any)
            ...
            <div className="flex min-h-screen w-full bg-dark1">
                <div className="w-full flex flex-col items-center justify-center px-4 text-center">
                    <div className="md:w-96 w-full max-w-md">
                        ...
                        <Link
                            href={route("forgot-password")} // Adjust route name if different
        ...
        <div className="flex min-h-screen w-full bg-dark1">
            <div className="w-full flex flex-col items-center justify-center px-4">
                ...
                >
                    ...
                    {/* General Error (from flash or server) */}
                    {errors.general && (
                            ...
                            >
                                {showPassword ? (
                                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                                        ...
                                        <path fillRule="evenodd" clipRule="evenodd" d="M2.458 12c1.65-4.59 5.61-7.5 9.542-7.5 3.932 0 7.892 2.91 9.542 7.5-1.65 4.59-5.61 7.5-9.542 7.5-3.932 0-7.892-2.91-9.542-7.5zm9.542-5.5c-3.26 0-6.59 2.36-8.042 6.5 1.452 4.14 4.782 6.5 8.042 6.5 3.26 0 6.59-2.36 8.042-6.5-1.452-4.14-4.782-6.5-8.042-6.5z" fill="currentColor" />
                                    </svg>
                                ) : (
                                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                                        <path d="M3.707 2.293a1 1 0 00-1.414 1.414l18 18a1 1 0 001.414-1.414l-18-18z" fill="currentColor" />
                                        <path fillRule="evenodd" clipRule="evenodd" d="M12 6c-3.932 0-7.892 2.91-9.542 7.5a1 1 0 001.884.684c1.452-4.14 4.782-6.5 8.042-6.5 3.26 0 6.59 2.36 8.042 6.5a1 1 0 001.884-.684C20.892 8.91 16.932 6 12 6z" fill="currentColor" />
                                        <path d="M12 10a2 2 0 00-2 2 1 1 0 11-2 0 4 4 0 014-4 1 1 0 010 2z" fill="currentColor" />
                                    </svg>
                                )}
                            </button>
                        </div>
